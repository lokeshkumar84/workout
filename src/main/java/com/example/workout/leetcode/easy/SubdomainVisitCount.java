...
public class SubdomainVisitCount{
    ...
    public static void main(String[] args){
        ...
        List<String> output = subdomainVisits(inputs);
        output.forEach(i -> System.out.println(i));
    }

    private static List<String> subdomainVisits(String[] inputs){
        ...
        for(String input:inputs){
            ...
            for(int j=domainSplit.length-1;j>=0;j--){
                ...
                key = domainSplit[j] + (j < domainSplit.length-1 ?".":"") + key;

                if(consolidatedMap.containsKey(key)){
                    consolidatedMap.put(key,consolidatedMap.get(key)+count);
                }else{
                    consolidatedMap.put(key,count);
                }

            }
        }
        List<String> result = new ArrayList<String>();
        consolidatedMap.forEach((k,v)->result.add(String.valueOf(v)+" "+k));
